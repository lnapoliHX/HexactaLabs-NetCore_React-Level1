...
const ProductTypesView = props => {
  return (
    <Container fluid>
      <div className="block-header">
        <h1>{props.productTypes.name}</h1>
      </div>
      <div className="info-box">
        <Row>
          <Col lg="2">Id</Col>
          <Col>{props.productTypes.id}</Col>
        </Row>
        <Row>
          <Col lg="2">Iniciales</Col>
          <Col>{props.productTypes.initials}</Col>
        </Row>
        <Row>
          <Col lg="2">Descripción</Col>
          <Col>{props.productTypes.description}</Col>
        </Row>
      </div>
      <div className="productTypes-view__button-row">
        <Button title="Editar" aria-label="Editar"
          className="productTypes-form__button"
          color="primary"
          onClick={() =>
            props.push(`/productTypes/update/${props.match.params.id}`)
          }
        >
        ...
        <Button title="Eliminar" aria-label="Eliminar"
          ...
          color="danger"
          onClick={() =>
            props.push(`/productTypes/view/${props.match.params.id}/remove`)
          }
        >
        ...
        <Button title="Volver" aria-label="Volver"
          className="productTypes-form__button btn-outline-secondary"
          color="default"
          onClick={() => props.push(`/productTypes`)}
        >
